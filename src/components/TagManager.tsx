...
export default function TagManager({ open, onClose }: Props) {
  ...
  function cancelEdit() {
    setEditId(null);
    setDraft({ code: '', name: '', category: '', color: '#FF9900' });
  }
  function saveEdit() {
    if (!editId) return;
    updateTag(editId, draft);
    cancelEdit();
  }
  function add() {
    if (!draft.code.trim()) return;
    addTag({ ...draft, code: draft.code.toUpperCase() });
    setDraft({ code: '', name: '', category: '', color: draft.color });
  }
      ...
      <div style={{background:'#fff', width:900, maxHeight:'85vh', overflow:'auto', borderRadius:10, padding:16, boxShadow:'0 10px 30px rgba(0,0,0,0.3)'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'90px 220px 180px 1fr', gap:10, alignItems:'center', marginBottom:10}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(10, 22px)', gap:6}}>
              {colors.map(c => (
                <button key={c} title={c} onClick={()=>setDraft(d=>({...d, color:c}))}
                        style={{
                          width:20, height:20, borderRadius:4, border: `2px solid ${draft.color===c?'#333':'#ccc'}`,
          ...
          <div>
            ...
            <input value={draft.category} onChange={e=>setDraft(d=>({...d, category:e.target.value}))}
                   placeholder="Lights / Switches / Receptacles ..." style={{width:'100%'}} />
          </div>

          <div>
            <div style={{marginBottom:6}}>Name</div>
            <input value={draft.name} onChange={e=>setDraft(d=>({...d, name:e.target.value}))}
                   placeholder="Fixture A1 - 2x4 LED" style={{width:'100%'}} />
          </div>
        </div>
        ...
        <table style={{width:'100%', borderCollapse:'collapse', fontSize:14}}>
          ...
          <tbody>
            {tags.map(t => (
              <tr key={t.id} style={{borderBottom:'1px solid #f2f2f2'}}>
                <td style={{padding:'8px 4px'}}>
                  <div style={{width:20, height:20, borderRadius:4, background:t.color, border:'1px solid #999'}}/>
                </td>
            ...
            {tags.length === 0 && (
              <tr><td colSpan={5} style={{padding:12, color:'#666'}}>No tags yet. Add or import a JSON database.</td></tr>
            )}
          </tbody>
        </table>
      </div>
