...
/** Robust base64 -> ArrayBuffer (handles large strings) */
async function b64ToArrayBuffer(b64: string): Promise<ArrayBuffer> {
  try {
    // Fast path
    const bin = atob(b64);
    ...
    // Fallback: use fetch(data:) which avoids atob size quirks
    const res = await fetch('data:application/octet-stream;base64,' + b64);
    return await res.arrayBuffer();
  }
}

/** Simple ArrayBuffer -> base64 for Open PDF */
...
export default function App() {
  ...
  async function doOpenProject(file: File) {
    ...
    try {
      ...
      // Ensure header shows the project name we saved
      try {
      ...
      // Restore PDF (robust base64 decoder)
      if (bundle.pdf && typeof bundle.pdf.bytesBase64 === 'string' && bundle.pdf.bytesBase64.length > 0) {
        try {
          const ab = await b64ToArrayBuffer(bundle.pdf.bytesBase64);
        ...
        // No embedded PDF — project opens; user can open the matching PDF
        setPdf(null);
        setFileName('');
      }
    ...
    <div className="app_root" style={{display:'flex', flexDirection:'column', height:'100vh'}}>
      ...
      <div style={{display:'flex', alignItems:'center', gap:12, padding:'8px 12px', background:'#0d3b66', color:'#fff', position:'sticky', top:0, zIndex:50}}>
        ...
        <input
          ...
          style={{display:'none'}}
          onChange={async (e)=>{ const f = e.target.files?.[0]; if (f) await doOpenProject(f); e.currentTarget.value=''; }}
        />
