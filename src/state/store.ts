...
export const useStore = create<State>((set, get) => ({
  ...
  addObject: (pageIndex, obj) => {
    ...
    set(s => {
      const pages = s.pages.map(p => p.pageIndex === pageIndex ? ({...p, objects:[...p.objects, obj]}) : p);
      return { pages };
    });
  },
  ...
  replaceObjects: (pageIndex, objs) => {
    const { pushHistory } = get(); pushHistory(pageIndex);
    set(s => {
      const pages = s.pages.map(p => p.pageIndex === pageIndex ? ({...p, objects: objs}) : p);
      return { pages };
    });
  },
  ...
  deleteSelected: (pageIndex) => {
    const { pushHistory } = get(); pushHistory(pageIndex);
    set(s => {
      const pages = s.pages.map(p => p.pageIndex !== pageIndex ? p : ({...p, objects: p.objects.filter(o => !s.selectedIds.includes(o.id))}));
